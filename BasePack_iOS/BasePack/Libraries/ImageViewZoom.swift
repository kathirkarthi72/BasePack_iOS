...
open class ImageViewZoom: UIScrollView {
    ...
    @objc open func load(image: String) {
        ...
        zoomView?.load(from: image, handler: { (downloadedImage) in
            
            self.zoomView = UIImageView(image: downloadedImage)
            ...
            self.configureImageForSize(downloadedImage?.size ?? self.frame.size)
        })
    }
