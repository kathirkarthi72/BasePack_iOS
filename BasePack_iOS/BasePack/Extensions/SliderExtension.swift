...
//        thumbTextLabel.text = "\(Int(value))" //.roundTo(places: 1).description
        
        let dateAttributedString: NSMutableAttributedString = NSMutableAttributedString(string: thumbTextLabel.text ?? "")
        dateAttributedString.setColorForText(textForAttribute: "\(thumbTextLabel.accessibilityHint ?? "")", withColor: ColorManager.white.color, withFont: UIFont.customFont(.ExoRegular, size: 8.0))
...
//        let tRect = thumbRect
        if isHideTooltip {
            ...
            let popupRect = thumbFrame.offsetBy(dx: 0, dy: -(thumbFrame.size.height * 1.3))
    ...
    override func awakeFromNib() {
        ...
        thumbTextLabel.backgroundColor = ColorManager.greenTheme.color
        thumbTextLabel.font = UIFont.customFont(.ExoSemiBold, size: 12.0)
    ...
    private func initToolTip() {
        ...
        toolTip?.fillColor = ColorManager.greenTheme.color
        toolTip?.textColor = ColorManager.white.color
        toolTip?.draw(.zero)
        toolTip?.font = UIFont.customFont(.ExoMedium, size: 12.0)
